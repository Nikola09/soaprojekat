...
namespace Dashboard
{
	public partial class FormStorage : Form
	{
		...
		private List<Apii> apiis;
		private List<Battery> batteries;
		private List<Location> locations;
		private System.Timers.Timer timer1;
		private const int reloadTime = 5000;
		private string url = "http://localhost:8882/"; //CHECK
		...
		{ 
			...
			if (response.IsSuccessStatusCode)
			{
				...
				var dataObjects = JsonConvert.DeserializeObject<List<Ambient>>(response.Content.ReadAsStringAsync().Result);  //Make sure to add a reference to System.Net.Http.Formatting.dll
			...
			{
				var dataObjects = JsonConvert.DeserializeObject<List<Apii>>(response2.Content.ReadAsStringAsync().Result);
			...
			{
				var dataObjects = JsonConvert.DeserializeObject<List<Battery>>(response3.Content.ReadAsStringAsync().Result);
			...
			{
				var dataObjects = JsonConvert.DeserializeObject<List<Location>>(response4.Content.ReadAsStringAsync().Result);
